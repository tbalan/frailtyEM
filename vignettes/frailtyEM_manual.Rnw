...
A slightly different parametrization is presented in \cite{hougaard2012analysis}, dependent on the parameter $\eta_H$. The tie-in is obtained by setting $\eta_H = (m+1) \theta$.
