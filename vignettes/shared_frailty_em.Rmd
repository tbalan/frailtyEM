...
## The likelihood
The type of data that these models can be applied on is usually consisting of either clustered failures or recurrent events. In both cases we will refer to a unit which has the same value of the frailty as a cluster. The process intensity of subject $j$ in cluster $i$ can be written as

$$
\lambda_{ij}(t) = z_i \exp(\gamma'\mathbf{x}_{ij}(t))\lambda_0(t)
$$

The likelihood for cluster $i$ can be written as
$$
L_i(\theta,\gamma,\lambda_0) = \prod_j \prod_{k} z_i \exp()
$$
