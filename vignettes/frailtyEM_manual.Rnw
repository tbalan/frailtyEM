...
The maximizer used is \code{optimize} from the \pkg{stats} library. This requires the specification of an interval to search for a maximum. The default search intervals, described by \code{emfrail_control()}, may be overriden by the user. Larger intervals provide more precision, however numeric problems may occur when the EM algorithm is ran in stranger parts of the parameter space. For instance, if the frailty variance of the gamma distribution is allowed to take very large values.
