...
dev.off()
@
\includegraphics[width = 16.5cm, height = 5.5cm]{cgd_pred}
...
@
For a hypothetical individual that changes treatment from placebo to \mbox{rIFN-g} at time 200, predictions may also be obtained, as shown in Figure~\ref{fig:predchange}.
...
pdf("cgd_treatdif.pdf", width = 4, height = 4)
p
dev.off()
@
\includegraphics[width = 8cm, height = 8cm]{cgd_treatdif}
\caption{Predicted conditional and marginal cumulative hazards for a male that switches treatment from placebo to \mbox{rIFN-g} at time 200}
...
The conditional and marginal hazard betw ratios from different distributions can also be visualized easily. We also fitted an IG frailty model on the same data, and the hazard ratio between two males from different treatment arms is shown in Figure~\ref{fig:hr}. While all models shrink the hazard ratio towards 1, it can be seen that this effect is slightly more pronounced for the gamma than for the IG, while the PS exhibits a constant ``average'' shrinkage. This type of behaviour from the PS is seen as a strength of the model \citep{hougaard2012analysis}.

<<cgd_ig, include=FALSE, echo=FALSE>>=
...
dev.off()
@
\includegraphics[width = 16.5cm, height = 5.5cm]{cgd_hr}
\caption{Conditional and marginal hazard ratio between two males from the placebo and \mbox{rIFN-g}  treatment arms  from the gamma, PS and IG frailty models}
\label{fig:hr}
\end{center}
\end{figure}
...
The newest features will be implemented in the development version of the package at \url{https://github.com/teddybalan/frailtyEM}.
