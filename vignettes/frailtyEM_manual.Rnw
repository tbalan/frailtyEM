...
The consequences of the semi-parametric model, where $\lambda_0 >0$ only at event time points, are that the $P(T > t) = 1$ for every $t$ before the first event time point. To assign a value for $\mathbf{x}$ before the entry time is speculative. Therefore, we only consider this case when $\mathbf{x}_i$ is time constant.
...
Most ideas from \cite{nielsen1992counting} are used here. The crucial observations are that the E step involves calculating the empirical Bayes estimates of the frailties $\widehat{z}_i = E[Z_i | data]$.
...
In the maximization procedure, $\theta$ is introduced on the log-scale. That is for numerical stability (the interval for searching for the maximum likelihood is ``stretched'' to the real numbers) and for asymptotic normality. Although maximum likelihood estimates are asymptotically normal, the likelihood is likely to be skewed, especially if the maximum likelihood is close to the edge of the parameter space. In this case, the standard error of the estimate may be difficult to interpret for constructing confidence intervals. It has been shown that symmetric confidence intervals of $\log \theta$, translated to the scale of $\theta$, provide good coverage \cite{balan2016ascertainment}.

The maximizer used is \code{optimize} from the \pkg{stats} library. This requires the specification of an interval to search for a maximum. The default search intervals, described by \code{emrail_control()}, may be overriden by the user. Larger intervals provide more precision, however numeric problems may occur when the EM algorithm is ran in stranger parts of the parameter space. For instance, if the frailty variance of the gamma distribution is allowed to take very large values.
...
\subsection{Standard errors and return object}
\label{sec:return}
...
A method which is more stable, yet also more computationally intensive, for quantifying this uncertainty in the estimates, is through likelihood-based confidence intervals. This requires finding the $\theta$ values for which the difference between the maximum likelihood and the specific profile maximum likelihood values at $\theta$ equals a critical value, calculated from the $\chi^2(1)$ distribution, and is discusse din Appendix A2. This can be achieved with a root-finding routine such as the \code{uniroot()} function in the \pkg{stats} package.

The standard error of the estimates for $\beta$ and $\lambda_0(\cdot)$ are calculated with Louis' formula \citep{louis1982finding}, for $\theta$ fixed to the maximum likelihood estimate. The corresponding information matrix leads to an underestimate of the standard errors, because it does not take into account the uncertainty in estimating $\theta$. An adjustment is performed by calculating the information matrix for $\beta$ and $\lambda_0$ also at $\hat \theta \pm \varepsilon $. The details of this are described in Appendix A3.
...
\subsection{cgd}
The data are from a placebo controlled trial of gamma interferon in chronic granulotomous disease (CGD). It contains the time to recurrence of serious infections observed through end of study for each patient.
<<bladder1>>==
data(cgd)
@
The variables of interest here are: \code{tstart}, \code{tstop} and \code{status} determine the outcome. The individual is identified by the variable \code{id}. For the purpose of illustration, we will use `treat` (treatment or placebo) and `sex` (female or male) as covariates, althought a larger number of variables are recorded in the data set.
...
The results are almost identical to a gamma frailty fit from \code{coxph}. The marginal log-likelihood in the \code{emfrail} object is slightly higher, that is because the estimation of the frailty distribution is more precise. In addition, \code{emfrail} also provides a 95\% confidence interval for the frailty variance.
<<bladder2_asd>>==
...
The empirical Bayes frailty estimates are also identical for the two ways of fitting the model.
...
To look at predicted cumulative hazard curves we take two individuals, one from the treatment arm and one from the placebo arm, both males.
...
The coefficient estimates are similar with those of \code{m1}. The ``frailty summary'' part is quite different though. The positive stable distribution has infinite expectation. However, Kendall's $\tau$ is easily obtained, and in this case it is smaller than in the gamma frailty model. Unlike the gamma or PVF distributions, the positive stable frailty predicts a marginal model with proportional hazards where the marginal hazard ratios are an attenuated version of the conditional hazard ratios shown in the output. The attenuation factor, shown in the output, is calculated as described in Appendix A1. This is discussed at length in \cite{hougaard2012analysis} and it can be easily visualized with \code{emfrail}.
<<bladder2_hazardratios, fig=TRUE>>==
par(mfrow=c(1,2))
plot_hr(m1,
        newdata = data.frame(treat = c("placebo", "rIFN-g"), sex = c("male", "male")),
        main = "gamma")

plot_hr(m2,
        newdata = data.frame(treat = c("placebo", "rIFN-g"), sex = "male"),
        main = "stable")
...
The data, presented originally in \cite{mcgilchrist1991regression}, contains the time to infection for kidney patients using  a portable dialysis equipment. The infection may occur at the insertion of the catheter and at that point, the catheter must be removed, the infection cleared up, and the catheter reinserted. Each of the 38 patients has exactly 2 observations, representing the time from insertion until the next infection. There are 3 covariates: sex, age and a disease (a factor with 4 levels). A data analysis based on frailty models is described in \citet[ch.~9.5.2]{survival-book}. The authors note that, when \code{disease} is included in the model, a gamma frailty model  offers no evidence of heterogeneity. However, when \code{disease} is removed from the model, then there seems to be moderate evidence for heterogeneity. This is an example where the frailty may be interpreted as a missing covariate.
<<kidney1>>==
data(kidney)
...
$$
For identifiability, the restriction $\alpha = 1$ is made; $E[Y]$ is undefined and $\mathrm{Var}Y = \infty$.
...
Furthermore, $\mathrm{E} \log Y = - (\gamma^{-1} - 1) \psi(1)$ and
...
$$
where $\mathrm{sign}$ denotes the sign. This is the same parametrizaion as in \cite{aalen2008survival}. The parameters from  \cite{hougaard2012analysis} are defined by the correspondences $m = -\alpha_H$, $\gamma = \theta_H$, $\alpha = \vert \delta_H/m \gamma^{-m} \vert$.
We have
$$
...
$$
For identifiability, we set the $\mathrm{E} Y = 1$. The parametrization is, for a parameter $\theta >0$,
$\gamma = (m + 1) \theta$ and
$\alpha = \frac{m + 1}{m} \theta$. This results in $\mathrm{Var} Y = \theta^{-1}$.
...
By inverting $\mathcal{I}_{\theta, \theta}$ and $\mathcal{I}_{\eta,\eta}$, two variance-covariance matrices are obtained: $\mathrm{Var}(\theta)$ and $\mathrm{Var}(\eta | \theta = \widehat\theta)$. The latter matrix is the variance-covariance for $\eta$ under fixed $\theta$; from this, the standard errors are calculated, for example, in the \pkg{survival} package. This leads to an under-estimation of the standard errors, the extent of which, to our knowledge, has not been yet studied.
...
\section*{Appendix A4: confidence intervals for the frailty parameters}
The former relies on constructing a (non-symmetric) 95\% confidence interval for $\log \theta$ on the profile log-likelihood scale \eqref{eq:profilelik} while the delta method-based is based on constructing a symmetric 95\% confidence interval for $\log \theta$ on the profile log-likelihood scale based on the asymptotic normality of the estimate.
