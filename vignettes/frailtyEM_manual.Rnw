...
<<cgd2>>==
m1 <- emfrail(Surv(tstart, tstop, status) ~ sex + treat + cluster(id),
  data = cgd)
@
...
The results are almost identical to a gamma frailty fit from \code{coxph}. The marginal log-likelihood in the \code{emfrail} object is slightly higher, because the estimation of the parameters of the frailty distribution is more precise. In addition, \code{emfrail} also provides a 95\% confidence interval for the frailty variance.
<<cgd2_coxph>>==
...
<<bladder2_stable>>==
m2 <- emfrail(Surv(tstart, tstop, status) ~ treat + sex + cluster(id),
  data = cgd,
